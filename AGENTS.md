...
This project uses [graphify](https://github.com/.../graphify) to maintain a persistent knowledge graph in `graphify-out/graph.json`.

### Workflow

1. **Before reading source files**, query the graph for context:
   - Search for relevant nodes by name or type
   - Use BFS/DFS to understand relationships between files
   - This saves tokens by avoiding unnecessary file reads

2. **After making changes**, regenerate the graph:
   ```
   /git-graphify --update
   ```
   This refreshes `graphify-out/graph.json` with the latest file contents, imports, and dependencies.

### Graph structure

- Each source file is a node
- `import` / `export` relationships are edges
- Community detection groups related modules (commands, utils, types)
- God nodes represent high-level concepts (CLI commands, tokwatchr API, kowu-cli API)
